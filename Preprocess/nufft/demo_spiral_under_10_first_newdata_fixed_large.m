...
for ie = 1:Neco
    DCF = dcR10(:,:,ie)./ max(max(dcR10(:,:,ie)));
    par.FT{ie} = NUFFTDCF(trj_kxy(:,1,1,ie), ones(size(DCF.^2)), shift, [120, 120]);
end
...
for ip = 1:Npar
    ...
    for ie = 1:Neco
        ...
        for ic = 1:Ncoil
            ...
            img_uncombined(ic,:,:,ie) = par.FT{ie}' * (DCF.*cast(squeeze(data_full_reshaped(:,ie,1,ip,ic)), par.prec));          
        end
...
%    mask_2 = mask_1;
    ...
    fname = sprintf('/home/naxos2-raid7/hongygu/non_cartesian/spiral_fmri/DCF_modified_results/my_training/ssdu_0_5/new_run/fixed_data/large_testing_R10/slice_%d.mat', count);%+60);
