...
Npar = 72;
img_uncombined_train = zeros([1, [120,120], Ncoil, Neco, Npar]);
img_uncombined_validation = zeros([1, [120,120], Ncoil, Neco, Npar]);
data_single_reshaped = reshape(data_single, [2176, 6, 72, Ncoil]);

training_data = zeros(1, 2176, 6, 72, Ncoil);
validation_data  = zeros(1,2176, 6, 72, Ncoil);

m = 10;
training_data(1, :,:,:,:) = data_single_reshaped(:, :, :,:,:);
training_data(1, training_all(m,:)~=1,:,:,:) = 0;
validation_data(1, :,:,:,:) = data_single_reshaped(:, :, :,:,:);
validation_data(1, training_all(m,:)==1,:,:,:) = 0;
...
for ip = 1:Npar
    for ie = 1:Neco
        ...
        for ic = 1:Ncoil
            ...
            img_uncombined_train(1, :,:,ic,ie,ip) = par.FT{m, ie}' * (DCF.*cast(squeeze(training_data(1, :,ie,ip,ic)), par.prec));          
            img_uncombined_validation(1, :,:,ic,ie,ip) = par.FT{m+10, ie}' * (DCF.*cast(squeeze(validation_data(1, :,ie,ip,ic)), par.prec));
        end
    end
end
