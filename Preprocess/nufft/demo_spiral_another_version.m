...
load('/home/naxos2-raid7/hongygu/non_cartesian/spiral_fmri/RawData/full_sb1.mat')
...
for ie = 1:Neco
    ...
    par.FT{ie} = NUFFTDCF(trj_kxy(:,:,1,ie), ones(size(DCF)), shift, [120, 120]);
end
...
for ip = 1:Npar
    for ie = 1:Neco
        ...
        for ic = 1:Ncoil
            ...
            img_uncombined(:,:,ic,ie,ip) = par.FT{ie}' * (DCF.*cast(squeeze(data_full_reshaped(:,ie,:,ip,ic)), par.prec));          
        end
    end
end
