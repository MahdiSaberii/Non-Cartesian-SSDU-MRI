...
DCF1 = dcR5(:,:,1,1)./ max(max(dcR5(:,:,1,1)));
DCF2 = dcR5(:,:,1,1);
...
for ie = 1:Neco
    par.FT{ie} = NUFFTDCF(traj, ones(size(traj)), shift, [240, 240]);
end
...
for ie = 1:Neco
    ...
    intermediate = DCF.* (par.FT{ie} * delta_mat1);
