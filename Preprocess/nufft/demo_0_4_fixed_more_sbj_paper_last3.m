...
for ie = 1:Neco
    trj = trj_kxy(:, 1, ie);
    DCF = dc10x(:,ie)./ max(max(dc10x(:,ie)));
    par.FT{m, ie} = NUFFTDCF(trj, ones(size(DCF.^2)), shift, [80, 80]);
end
end
...
for ie = 1:Neco
    trj = trj_kxy(:, 1, ie);
    DCF = dc10x(:,ie)./ max(max(dc10x(:,ie)));
    par.FT{m+10, ie} = NUFFTDCF(trj, ones(size(DCF.^2)), shift, [80, 80]);
end
end

Ncoil = size(data_single, 3); %different
Npar = 24;
img_uncombined_train = zeros([Ncoil, [80,80],Neco]);
img_uncombined_validation = zeros([Ncoil, [80,80], Neco]);
...
for m = 1: 10
    ...
    atb_2_train = zeros(80,80,Neco);
    atb_2_val = zeros(80,80,Neco);
