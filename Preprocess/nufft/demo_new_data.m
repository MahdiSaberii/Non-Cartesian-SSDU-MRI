...
load('/home/naxos2-raid7/hongygu/non_cartesian/spiral_fmri/RawData/full_sbj1_2D.mat')
...
for ie = 1:Neco
    ...
    par.FT{ie} = NUFFTDCF(trj_kxy(:,1:5:end,1,ie), ones(size(DCF.^2)), shift, [120, 120]);
end
...
data_full_reshaped = reshape(data_full, [2176, 6, 10, 72, Ncoil]);
...
for ip = 1:Npar
    for ie = 1:Neco
        ...
        for ic = 1:Ncoil
            ...
            img_uncombined(:,:,ic,ie,ip) = par.FT{ie}' * (DCF.*cast(squeeze(data_full_reshaped(:,ie,1:5:end,ip,ic)), par.prec));          
        end
    end
end
