...
figure()
scatter(kx, ky, 4, 'filled','white')
xlim([-0.6 0.6])
ylim([-0.6 0.6])
set(gca,'color','black');

traj_train1 = traj(training_all(1, :) == 1,:);
traj_val1 = traj(training_all(1, :) == 0,:);

traj_train2 = traj(training_all(2, :) == 1,:);
traj_val2 = traj(training_all(2, :) == 0,:);

traj_train3 = traj(training_all(3, :) == 1,:);
traj_val3 = traj(training_all(3, :) == 0,:);

figure()
scatter(real(traj_train1), imag(traj_train1), 4, 'filled','white')
xlim([-0.6 0.6])
ylim([-0.6 0.6])
set(gca,'color','black');

figure()
scatter(real(traj_val1), imag(traj_val1), 4, 'filled','white')
xlim([-0.6 0.6])
ylim([-0.6 0.6])
set(gca,'color','black');

figure()
scatter(real(traj_train2), imag(traj_train2), 4, 'filled','white')
xlim([-0.6 0.6])
ylim([-0.6 0.6])
set(gca,'color','black');

figure()
scatter(real(traj_val2), imag(traj_val2), 4, 'filled','white')
xlim([-0.6 0.6])
ylim([-0.6 0.6])
set(gca,'color','black');

figure()
scatter(real(traj_train3), imag(traj_train3), 4, 'filled','white')
xlim([-0.6 0.6])
ylim([-0.6 0.6])
set(gca,'color','black');

figure()
scatter(real(traj_val3), imag(traj_val3), 4, 'filled','white')
xlim([-0.6 0.6])
ylim([-0.6 0.6])
set(gca,'color','black');
...
%%
load('/home/naxos2-raid7/hongygu/non_cartesian/spiral_fmri/DCF_modified_results/my_training/ssdu_0_5/new_run/fixed_data/1_2_3_4_5_6/M.mat')

example_M = squeeze(M(3,1,:,:));

figure()
imshow(abs(example_M), [0 1])

imwrite(abs(example_M), '/home/naxos2-raid7/hongygu/non_cartesian/spiral_fmri/DCF_modified_results/my_training/ssdu_0_5/new_run/fixed_data/M3.png')

%%
%%
load('/home/naxos2-raid7/hongygu/non_cartesian/spiral_fmri/DCF_modified_results/my_training/ssdu_0_5/new_run/fixed_data/1_2_3_4_5_6/figure.mat')
...
par = 44;
train1 = par.FT{m, ie}' * (DCF.*cast(squeeze(training_data(m, :,ie,ip,ic)), par.prec));          
valid1 = par.FT{m+5, ie}' * (DCF.*cast(squeeze(validation_data(m, :,ie,ip,ic)), par.prec));
