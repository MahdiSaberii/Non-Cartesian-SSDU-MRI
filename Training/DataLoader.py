...
class DataLoaderSL(Dataset):    
    ...
    def __getitem__(self , index):
      # Theta, Lambda
      Theta_EHWy  = sio.loadmat(self.datapath_theta[index])[f"theta_ehy_6"].transpose([2,0,1,3])     # Original Data shape: [120,120,   5,7]  == [img_x,img_y,        echoes,Masks]
      Lambda_EHWy = sio.loadmat(self.datapath_theta[index])[f"lambda_ehy_6"].transpose([3,2,0,1,4])  # Original Data shape: [120,120,52,5,7]  == [img_x,img_y,n_coils,echoes,Masks]
      
      FileName        = self.datapath_theta[index].split('/')[-1]    
      ...
      CoilAddress = f"../Preprocess/Coils/{SubjectNumber}/slice_{SliceNumber}.mat"
      Coil        = sio.loadmat(CoilAddress)["sens_map"].transpose([2,0,1])   # Original Coil shape: [120,120,44] -> [44,120,120]
      
      
      # M Operator
      M_single = sio.loadmat(f"../Preprocess/M/M_single.mat")["M"] # [14,5,240,240] == [2*Masks, Echoes, 2*img_width, 2*img_width]
      M_full   = sio.loadmat(f"../Preprocess/M/M_full.mat")["M"]   # [5,240,240]    == [Echoes,          2*img_width, 2*img_width]
      M_single = torch.tensor(M_single)
      M_full   = torch.tensor(M_full)
      
      label_path = f"../Preprocess/Labels/"+ "subject_" + str(SubjectNumber) + "_slice_"+str(SliceNumber) + ".mat"
      label_ehy  = sio.loadmat(label_path)[f"label_ehy_temp"].transpose([2,0,1])  # Original Data shape: [120,120,5] 
      
      return Theta_EHWy, Lambda_EHWy, Coil , SliceNumber , SubjectNumber, TimeIndex, FileName, M_single,label_ehy, M_full
