...
for ie = 1:Neco
    ...
    par.FT{m, ie} = NUFFTDCF(trj_training(m,:,ie), DCF.^2, shift, [120, 120]);
end
end
...
for ie = 1:Neco
    ...
    par.FT{m+5, ie} = NUFFTDCF(trj_validation(m,:,ie), DCF.^2, shift, [120, 120]);
end
end
...
for ip = 1:Npar
    for ie = 1:Neco
        for ic = 1:Ncoil
            ...
            img_uncombined_train(m, :,:,ic,ie,ip) = par.FT{m, ie}' * cast(squeeze(training_data(m, :,ie,ip,ic)), par.prec);          
            img_uncombined_validation(m, :,:,ic,ie,ip) = par.FT{m+5, ie}' * cast(squeeze(validation_data(m, :,ie,ip,ic)), par.prec);
        end
    end
end
end
